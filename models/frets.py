...
class BackboneFreTS(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.n_features * self.n_steps * self.embed_size, self.hidden_size),
            nn.LeakyReLU(),
            # nn.Linear(self.hidden_size, self.n_pred_steps), # 将输出维度从n_pred_steps调整为num_classes
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)  # (batch_size, n_features, n_steps)
        x = x.unsqueeze(3)      # (batch_size, n_features, n_steps, 1)
        print(f"After permute and unsqueeze: {x.shape}")
        bias = x

        if self.channel_independence == "0":
            x = self.MLP_channel(x, *x.shape[:3])
            print(f"After MLP_channel: {x.shape}")

        x = self.MLP_temporal(x, *x.shape[:3])
        print(f"After MLP_temporal: {x.shape}")

        # Adjust bias shape to match x
        if bias.shape != x.shape:
            bias = bias[:, :, :x.shape[2], :]  # Truncate bias time steps
            bias = bias.expand_as(x)          # Expand bias feature dimension

        x = x + bias
        print(f"After adding bias: {x.shape}")
